...
class DOMBattle extends TeamBattle
{	
    constructor(system, name, map_id, time, max_people, min_rank, max_rank, limit, bonuses = true, autobalance = true, friendlyFire = false, pro = false) {
        super(system, name, map_id, "DOM", time, max_people, min_rank, max_rank, limit, bonuses, autobalance, friendlyFire, pro);
    ...
    extraditeScore(id, team, action) {
        ...
        for (var i = 0; i < users.length; i++) {
            ...
            if (action === "lost") {
                console.log('\x1b[35m%s\x1b[0m',' > ' + user.nickname + " получает " + lostScore + " опыта за захват точки");
                ...
                tank.sendScore();
				this.addFund((3 + 0.01 * user.rank) * oppositeTeamSize);
            }
            else if (action === "captured") {
                console.log('\x1b[35m%s\x1b[0m',' > ' + user.nickname + " получает " + capturedScore + " за снятие точки");
                user.score += capturedScore;
                tank.addScore(capturedScore * (tank.garage.hasItem("up_score") ? 1.5 : 1));
                var stats = this.getStatistics(tank.name);
                this.broadcast("update_player_statistic;" + JSON.stringify(stats));
                tank.sendScore();
                this.addFund((5 + 0.01 * user.rank) * oppositeTeamSize);
            }
        }
    }
